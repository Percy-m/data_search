<template>
  <div class="app-container">
            ...
            <div v-if="activeDashboard" class="board-view">
              ...
              >
                <grid-item
                  ...
                  :i="item.i"
                  class="widget-card"
                  @resized="handleWidgetResize(item.i)"
                >
    ...
    <!-- Dialogs remain essentially the same but with updated variables -->
    ...
    <!-- ... (Keep other existing dialogs like dsDialog, detailDialog, saveQueryDialog, thresholdDialog) ... -->
...
const editorSql = ref('SELECT o.country, sum(o.revenue) as total \nFROM bi_demo.orders o \nGROUP BY o.country')
...
const fetchDataSources = async () => {
  ...
  } catch (error) { ElMessage.error('获取数据源失败') }
}
...
const saveDataSource = async () => {
  ...
  try {
    ...
    dsDialogVisible.value = false; fetchDataSources()
...
const saveCurrentQuery = async () => {
  ...
  try {
    await axios.post(META_API_BASE + '/', { name: saveQueryName.value, raw_sql: editorSql.value, data_source_id: currentDataSourceId.value })
...
const loadQueryToEditor = (q) => {
  editorSql.value = q.raw_sql; currentDataSourceId.value = q.data_source_id; runEditorQuery()
}
...
const deleteDashboard = async (id) => {
  ElMessageBox.confirm('删除看板?', '警告', {type:'warning'}).then(async () => {
...
const addWidgetToDashboard = (query) => {
  ...
  const newWidget = {
    ...
    query_id: query.id, query_name: query.name, query_sql: query.raw_sql, query_thresholds: query.thresholds, data_source_id: query.data_source_id
  }
...
const handleWidgetResize = (i) => {
  // If we had echarts, we'd call chart.resize() here
}
...
const saveThresholds = async () => {
  ...
  try {
    ...
    // Update local layout state to reflect immediately
    activeDashboardLayout.value.forEach(w => {
...
const loadDetailData = async (colOrPage = 1) => {
  ...
  } catch (e) { ElMessage.error('加载失败') } finally { detailLoading.value = false }
}
