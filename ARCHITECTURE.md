...
#### 动态宏变量预编译 (Secure Macro Pre-Compilation)
为了解决数仓中业务表常常由于时间或版本分表（如 `orders_v1` 到 `orders_v2`）导致的 SQL 维护困难问题，平台实现了全局宏替换机制：
1. 分析师可以在编写固化查询时使用双大括号定义宏变量（如 `SELECT * FROM orders_{{version}} WHERE cty='{{my_country}}'`）。
2. 在前端看板通过全局控件输入映射关系字典并传递。
3. 后端在业务服务层 (`QueryService`) 将宏处理作为请求流的**前置守卫**。通过对宏传入的值进行极其严格的正则白名单约束（阻断单引号等 SQL 注入高危字符），实现安全的预编译文本替换。
4. 替换出纯净合法的 SQL 后，再向后投递给 `sqlglot` 等 AST 解析器，彻底避开了不同数据库方言解析器对 `{}` 符号可能会产生的语法树解析冲突。
...
### 3.1 核心依赖栈
*   **核心引擎**：Vue 3 (为了解决大宽表与巨量图表数据下 Grid Layout 拖拽卡顿的性能瓶颈，核心组件全面使用了 `shallowReactive` 与 `shallowRef`，为彻底解决大宽表与巨量图表数据下 Grid Layout 拖拽卡顿（包含拖拽组件 A 导致组件 B 挤压产生的级联重绘灾难），放弃了早期的“原生 DOM 劫持”黑客手段。采用**全屏级全局响应式事件屏蔽**：监听底层网格的交互事件（`@move`, `@resize`），一旦发生任何排版变动，全局进入交互态。利用顶层 CSS 强制隐藏所有图表的 DOM 树并替换为骨架屏。这切断了在复杂碰撞推挤过程中的任何浏览器 Layout 与 Paint 重算。交互结束后通过防抖延迟并向 Window 派发一次全局 Resize 事件，一劳永逸保证所有 ECharts 在 60FPS 拖拽后的完美尺寸复原。)
*   **UI 骨架**：Element Plus
