...
### 3.1 核心依赖栈
*   **核心引擎**：Vue 3 (为了解决大宽表与巨量图表数据下 Grid Layout 拖拽卡顿的性能瓶颈，核心组件全面使用了 `shallowReactive` 与 `shallowRef`，并在外层 `grid-layout` 上监听 `dragstart` 等早期事件构建了**交互态瞬时骨架屏降级**机制，同时辅以 Watcher 与全局 MouseUp 双重状态兜底，实现了零掉帧与零状态卡死的极致体验。)
*   **UI 骨架**：Element Plus
