...
### 核心设计亮点
- **抽象语法树 (AST) 查询**：放弃直接传递 SQL 的方式，采用 Pydantic 定义通用的查询请求体（Dimensions, Metrics, Filters）。这保证了业务层无需关心底层使用的是 ClickHouse 还是 MySQL。
- **通用下钻算法**：下钻被抽象为纯粹的数据结构转换操作（保留指标，替换维度，叠加路径上的过滤条件），这使得下钻功能与具体的业务表字段完全解耦，能够无缝适用任何多维数据模型。
